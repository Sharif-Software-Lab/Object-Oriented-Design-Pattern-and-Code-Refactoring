...
<div dir="rtl">
- Inline Class: این بازآرایی شامل ادغام کلاس Lazy با کلاسی است که بیشترین تعامل را با آن دارد، زیرا کلاس Lazy وظایف مستقلی انجام نمی‌دهد و وجودش ضروری نیست.
  
- Collapse Hierarchy: اگر کلاس Lazy بخشی از یک سلسله مراتب ارث‌بری است، می‌توان آن را با کلاس‌های والد یا فرزند ادغام کرد.
</div>
...
<div dir="rtl">
- Large Class (کلاس بزرگ): برخی از کلاس‌ها وظایف متعددی انجام می‌دهند و می‌توانند به بخش‌های کوچکتر تقسیم شوند.
  
- Long Method (متد طولانی): برخی متدها بیش از حد طولانی هستند و به وظایف متعدد پرداخته‌اند که نیاز به تجزیه به متدهای کوچکتر دارند.
- 
- Duplicate Code (کد تکراری): بخش‌های کد در قسمت‌های مختلف پروژه تکرار شده‌اند که می‌توانند با استفاده از توابع مشترک بهینه شوند.
- 
- Data Clumps (بهم چسبیدگی داده‌ها): گروه‌های متغیر که همواره با هم استفاده می‌شوند، باید به یک کلاس یا ساختار داده منتقل شوند.
- 
- Speculative Generality (کلی‌سازی بدون استفاده): برخی از کدها به صورت بیش از حد انتزاعی نوشته شده‌اند بدون اینکه نیاز واقعی به این انتزاع باشد.
- 
- Primitive Obsession (وابستگی به نوع‌های ابتدایی): استفاده زیاد از نوع‌های داده اولیه به جای اشیاء می‌تواند مشکلات نگهداری کد را افزایش دهد.
- 
- Feature Envy (حسادت ویژگی): برخی از متدها بیش از حد به داده‌ها و متدهای کلاس‌های دیگر دسترسی دارند، که نشان‌دهنده نیاز به بازآرایی است.
- 
- Inappropriate Intimacy (صمیمیت نامناسب): کلاس‌ها بیش از حد به جزئیات داخلی یکدیگر وابسته‌اند.
- 
- Lazy Class (کلاس تنبل): برخی از کلاس‌ها کار خاصی انجام نمی‌دهند و می‌توانند حذف یا با کلاس‌های دیگر ادغام شوند.
- 
- Temporary Field (فیلد موقتی): برخی از متغیرهای کلاس فقط در شرایط خاصی استفاده می‌شوند که نشان‌دهنده نیاز به انتقال این منطق به مکان مناسب‌تر است.
</div>
